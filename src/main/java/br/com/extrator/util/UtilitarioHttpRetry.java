        ...
        for (int tentativa = 1; tentativa <= maximoTentativas; tentativa++) {
            ...
            if (respostaAtual.statusCode() == 429) {
                ...
                if (tentativa < maximoTentativas) {
                    ...
                    LockSupport.parkNanos(TimeUnit.MILLISECONDS.toNanos(tempoDeEspera));
                } else {
    ...
    private static synchronized void aplicarThrottling(long throttlingMs) throws InterruptedException {
        ...
        if (tempoDesdeUltimaRequisicao < throttlingMs) {
            ...
            if (tempoDeEspera > 0) {
                ...
                UtilitarioHttpRetry.class.wait(tempoDeEspera);
            }
        }
    }

    /**
     * Executa uma requisição HTTP com retry para obter dados binários (ex: arquivos XLSX).
     * Aplica throttling, retry com backoff exponencial para erros 429.
     *
     * @param cliente              O cliente HTTP a ser usado.
     * @param fornecedorRequisicao Supplier que fornece uma nova instância da requisição a cada tentativa.
     * @param nomeOperacao         Nome da operação para logs.
     * @return A resposta HTTP com dados binários.
     * @throws IOException          Se ocorrer um erro de I/O irrecuperável.
     * @throws InterruptedException Se a thread for interrompida durante as pausas.
     */
    public static HttpResponse<byte[]> executarComRetryBytes(HttpClient cliente, Supplier<HttpRequest> fornecedorRequisicao, String nomeOperacao)
            throws IOException, InterruptedException {
        ...
        HttpResponse<byte[]> resposta = null;
        ...
        for (int tentativa = 1; tentativa <= maximoTentativas; tentativa++) {
            ...
            logger.debug("Enviando requisição para '{}' (Tentativa {}/{})", nomeOperacao, tentativa, maximoTentativas);
            HttpResponse<byte[]> respostaAtual = cliente.send(requisicao, HttpResponse.BodyHandlers.ofByteArray());
