...
public final class PipelineOrchestrator {
    ...
    private static final AtomicInteger PARALLEL_THREAD_COUNTER = new AtomicInteger(0);
    ...
    private List<StepRunOutcome> executarCoreEmParalelo(final PipelineStep stepA,
        ...
        final ExecutorService executor = criarExecutorParalelo();
        try {
            final LocalDateTime startedAtA = LocalDateTime.now();
            final LocalDateTime startedAtB = LocalDateTime.now();
            ...
            final StepRunOutcome outcomeA = aguardarOutcomeParalelo(stepA, futureA, startedAtA);
            final StepRunOutcome outcomeB = aguardarOutcomeParalelo(stepB, futureB, startedAtB);
    ...
    private ExecutorService criarExecutorParalelo() {
        return Executors.newFixedThreadPool(2, runnable -> {
            final Thread thread = new Thread(runnable, "pipeline-core-" + PARALLEL_THREAD_COUNTER.incrementAndGet());
    ...
    private void encerrarExecutor(final ExecutorService executor) {
        executor.shutdown();
        try {
            if (!executor.awaitTermination(ConfigEtl.obterTimeoutShutdownExecutorMs(), TimeUnit.MILLISECONDS)) {
    ...
    private StepRunOutcome aguardarOutcomeParalelo(final PipelineStep step,
        ...
        try {
            return future.get(obterTimeoutAguardoParaleloMs(step), TimeUnit.MILLISECONDS);
        } catch (final TimeoutException e) {
            future.cancel(true);
            return criarFalhaIrrecuperavel(
                step,
                startedAt,
                new ExecutionTimeoutException(
                    "Timeout ao aguardar conclusao paralela do step " + valorOuUnknown(step.obterNomeEtapa())
                        + " apos " + step.obterTimeoutExecucao().toMillis() + " ms",
                    e
                )
            );
        } catch (final InterruptedException e) {
            future.cancel(true);
            Thread.currentThread().interrupt();
            return criarFalhaIrrecuperavel(step, startedAt, e);
        } catch (final ExecutionException e) {
            future.cancel(true);
            final Throwable causa = e.getCause() == null ? e : e.getCause();
            return criarFalhaIrrecuperavel(step, startedAt, causa);
        }
    }

    private long obterTimeoutAguardoParaleloMs(final PipelineStep step) {
        final long baseMs = Math.max(1L, step.obterTimeoutExecucao().toMillis());
        final long graceMs = Math.max(0L, ConfigEtl.obterTimeoutParallelGraceMs());
        if (baseMs > (Long.MAX_VALUE - graceMs) / 2L) {
            return Long.MAX_VALUE;
        }
        return (baseMs * 2L) + graceMs;
    }

    private StepRunOutcome criarFalhaIrrecuperavel(final PipelineStep step,
                                                   final LocalDateTime startedAt,
                                                   final Throwable erroBruto) {
        ...
        final FailureMode modoResolvido = failurePolicy.resolver(entidade, taxonomy);
        final FailureMode modoAplicado = modoResolvido == FailureMode.RETRY
            ? FailureMode.CONTINUE_WITH_ALERT
            : modoResolvido;
        final StepStatus status = modoAplicado == FailureMode.DEGRADE ? StepStatus.DEGRADED : StepStatus.FAILED;
        final StepExecutionResult failure = StepExecutionResult.builder(stepName, entidade)
            ...
            .metadata("failure_mode", modoAplicado.name())
            .metadata("retry_suppressed", modoResolvido == FailureMode.RETRY)
            .build();
        ...
        logger.logarEstruturado("pipeline.step.failure", Map.of(
            ...
            "failure_mode", modoAplicado.name(),
            "mensagem", failure.getMessage()
        ));
        return new StepRunOutcome(failure, modoAplicado == FailureMode.ABORT_PIPELINE);
    }
    ...
    private StepRunOutcome executarStepComPoliticas(final PipelineStep step,
        ...
        try {
            final StepExecutionResult result = OperationTimeoutGuard.executar(
                stepName,
                step.obterTimeoutExecucao(),
                () -> retryPolicy.executar(
    ...
    private StepRunOutcome tratarFalhaStep(final PipelineStep step,
        ...
        final ErrorTaxonomy taxonomy = errorClassifier.classificar(erro);
        final FailureMode modoResolvido = failurePolicy.resolver(entidade, taxonomy);
        final FailureMode modo = (modoResolvido == FailureMode.RETRY && taxonomy == ErrorTaxonomy.TIMEOUT)
            ? FailureMode.CONTINUE_WITH_ALERT
            : modoResolvido;
        ...
        StepExecutionResult failure = StepExecutionResult.builder(stepName, entidade)
            ...
            .metadata("retry_suppressed", modoResolvido == FailureMode.RETRY && modo != FailureMode.RETRY)
            .build();

        if (modo == FailureMode.RETRY) {
            try {
                final StepExecutionResult retried = OperationTimeoutGuard.executar(
                    stepName + ":retry",
                    step.obterTimeoutExecucao(),
                    () -> step.executar(dataInicio, dataFim)
                );
