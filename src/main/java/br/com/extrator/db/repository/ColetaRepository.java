...
public class ColetaRepository extends AbstractRepository<ColetaEntity> {
    ...
    protected void criarTabelaSeNaoExistir(final Connection conexao) throws SQLException {
        if (!verificarTabelaExiste(conexao, NOME_TABELA)) {
            ...
            // Usar método que verifica se coluna existe antes de adicionar
            adicionarColunaSeNaoExistir(conexao, "cancellation_reason", "NVARCHAR(MAX)");
            adicionarColunaSeNaoExistir(conexao, "cancellation_user_id", "BIGINT");
            adicionarColunaSeNaoExistir(conexao, "cargo_classification_id", "BIGINT");
            adicionarColunaSeNaoExistir(conexao, "cost_center_id", "BIGINT");
            adicionarColunaSeNaoExistir(conexao, "destroy_reason", "NVARCHAR(MAX)");
            adicionarColunaSeNaoExistir(conexao, "destroy_user_id", "BIGINT");
            adicionarColunaSeNaoExistir(conexao, "invoices_cubed_weight", "DECIMAL(18, 3)");
            try {
            ...
            // Usar método que verifica se coluna existe antes de adicionar
            adicionarColunaSeNaoExistir(conexao, "notification_email", "NVARCHAR(255)");
            adicionarColunaSeNaoExistir(conexao, "notification_phone", "NVARCHAR(255)");
            adicionarColunaSeNaoExistir(conexao, "pick_type_id", "BIGINT");
            adicionarColunaSeNaoExistir(conexao, "pickup_location_id", "BIGINT");
            adicionarColunaSeNaoExistir(conexao, "status_updated_at", "NVARCHAR(50)");
