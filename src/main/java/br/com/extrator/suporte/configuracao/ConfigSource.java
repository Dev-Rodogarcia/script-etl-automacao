...
final class ConfigSource {
    ...
    static String obterConfiguracaoObrigatoria(final String nomeVariavelAmbiente) {
        ...
        if (valorSystemProperty != null && !valorSystemProperty.trim().isEmpty()) {
            logger.debug("Configuracao sensivel '{}' obtida da system property", nomeVariavelAmbiente);
        ...
        if (nomesVariaveisAmbiente != null) {
            for (int i = 0; i < nomesVariaveisAmbiente.length; i++) {
                ...
                if (valorSystemProperty != null && !valorSystemProperty.trim().isEmpty()) {
                    if (i > 0) {
                        logger.warn(
                            "Configuracao '{}' obtida por alias de system property '{}'. Prefira '{}'.",
                            nomeChaveProperties,
                            nomeVariavelAmbiente,
                            nomesVariaveisAmbiente[0]
                        );
                    } else {
                        logger.debug("Configuracao '{}' obtida da system property '{}'", nomeChaveProperties, nomeVariavelAmbiente);
                    }
                    return valorSystemProperty;
                }
