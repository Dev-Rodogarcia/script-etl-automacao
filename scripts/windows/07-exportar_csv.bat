...
if "%OPCAO%"=="2" (
    ...
    echo   5. fretes                ^(Fretes^)
    echo   6. manifestos            ^(Manifestos^)
    echo   7. localizacao_cargas    ^(Localizacao de Cargas^)
    echo   8. inventario            ^(Inventario / vw_inventario_powerbi^)
    echo   9. sinistros             ^(Sinistros / vw_sinistros_powerbi^)
    echo  10. raster_viagens        ^(Raster viagens^)
    echo  11. raster_viagem_paradas ^(Raster paradas^)
    echo  12. dim_usuarios          ^(Usuarios do Sistema - Dimensao^)
    ...
    if "!TABELA_NUM!"=="5" set "TABELA=fretes"
    if "!TABELA_NUM!"=="6" set "TABELA=manifestos"
    if "!TABELA_NUM!"=="7" set "TABELA=localizacao_cargas"
    if "!TABELA_NUM!"=="8" set "TABELA=inventario"
    if "!TABELA_NUM!"=="9" set "TABELA=sinistros"
    if "!TABELA_NUM!"=="10" set "TABELA=raster_viagens"
    if "!TABELA_NUM!"=="11" set "TABELA=raster_viagem_paradas"
    if "!TABELA_NUM!"=="12" set "TABELA=dim_usuarios"
