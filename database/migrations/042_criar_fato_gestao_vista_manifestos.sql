PRINT 'Migration 042: criar fato materializada de Gestao a Vista para manifestos';
GO
...
DECLARE @MigrationId NVARCHAR(255) = N'042_criar_fato_gestao_vista_manifestos';
...
BEGIN
    PRINT 'Migracao 042_criar_fato_gestao_vista_manifestos ja aplicada. Nenhuma acao necessaria.';
    RETURN;
END;

IF OBJECT_ID(N'dbo.manifestos', N'U') IS NULL
    THROW 51070, 'Tabela dbo.manifestos nao encontrada. Execute as migrations base antes da 042.', 1;

IF OBJECT_ID(N'dbo.coletas', N'U') IS NULL
    THROW 51071, 'Tabela dbo.coletas nao encontrada. Execute as migrations base antes da 042.', 1;

IF OBJECT_ID(N'dbo.fretes', N'U') IS NULL
    THROW 51072, 'Tabela dbo.fretes nao encontrada. Execute as migrations base antes da 042.', 1;
...
VALUES (
    @MigrationId,
    N'Cria fato materializada de manifestos para substituir agregacao pesada com OPENJSON sob demanda no Dashboard.'
);

PRINT 'Migration 042_criar_fato_gestao_vista_manifestos concluida com sucesso.';
GO
