...
-- WITH L AS (SELECT entidade, registros_extraidos, ROW_NUMBER() OVER (PARTITION BY entidade ORDER BY timestamp_fim DESC) rn FROM log_extracoes), B AS (SELECT 'usuarios_sistema' e, COUNT(*) c FROM dim_usuarios UNION ALL SELECT 'coletas', COUNT(*) FROM coletas WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'fretes', COUNT(*) FROM fretes WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'manifestos', COUNT(*) FROM manifestos WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'cotacoes', COUNT(*) FROM cotacoes WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'localizacao_cargas', COUNT(*) FROM localizacao_cargas WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'contas_a_pagar', COUNT(*) FROM contas_a_pagar WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'faturas_por_cliente', COUNT(*) FROM faturas_por_cliente WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE()) UNION ALL SELECT 'faturas_graphql', COUNT(*) FROM faturas_graphql WHERE data_extracao >= DATEADD(HOUR,-24,GETDATE())) SELECT COALESCE(l.entidade,b.e) AS entidade, l.registros_extraidos AS api, b.c AS banco, COALESCE(l.registros_extraidos,0)-COALESCE(b.c,0) AS diff, CASE WHEN l.registros_extraidos IS NULL THEN 'SEM_LOG' WHEN b.c IS NULL THEN 'SEM_BANCO' WHEN l.registros_extraidos=b.c THEN 'OK' WHEN ABS(l.registros_extraidos-b.c)<=5 THEN 'DIF_PEQ' ELSE 'DIF_GRANDE' END AS status FROM (SELECT * FROM L WHERE rn=1) l FULL OUTER JOIN B ON l.entidade=b.e ORDER BY status, entidade;
--
...
Banco AS (
    SELECT 'usuarios_sistema' AS e, COUNT(*) AS c FROM dim_usuarios
